...
发送验证码短信
*/
router.get('/peachlogin', function (req, res, next) {
  //1. 获取请求参数数据
...
短信登陆
*/
router.post('/Peachlogin', function (req, res, next) {
  ...
  UserModel.findOne({
    ...
    .catch(error => {
      console.error('/Peachlogin', error)
    })
...
router.get('/xxx', function (req, res) {
  setTimeout(function () {
    const data = require('../data/peachclassify.json')
    res.send({
      code: 0,
      data
    })
